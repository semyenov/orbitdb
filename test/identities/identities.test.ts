...
// rewrite import from @orbitdb/core
import {
	Identities,
	useIdentityProvider,
	getIdentityProvider,
...
describe('Identities', function () {
	...
	describe('verify identity\'s signature', () => {
		...
		it('false signature doesn\'t verify', async () => {
			...
			identity = await identities.createIdentity({ provider: FakeIdentityProvider()} as unknown as IdentityOptions)
	...
	describe('sign data with an identity', () => {
		...
		it('throws an error if private key is not found from keystore', async () => {
			...
			const modifiedIdentity = await Identity({ id: 'this id does not exist', publicKey, signatures, type } as IdentityInstance)
	...
	describe('manage identity providers', () => {
		...
		it('cannot add an identity provider with missing type', () => {
			...
			try {
				useIdentityProvider(NoTypeIdentityProvider as unknown as IdentityProvider)
			} catch (e) {
				err = (e as Error).toString()
			}
		...
		it('cannot add an identity provider with missing verifyIdentity', async () => {
			...
			try {
				useIdentityProvider(NoVerifyIdentityIdentityProvider as unknown as IdentityProvider)
			} catch (e) {
				err = (e as Error).toString()
			}
